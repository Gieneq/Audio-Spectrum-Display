...
void calc_energy_eval(int* bar_heights, double* bar_thrs, double* energy, double* burst_energy, double* mean_energy){
  ...
  *energy = double(sum_bar_thrs) / 3;
  burst_buffer[burst_end_index] = *energy;
  *burst_energy = burst_buffer[burst_end_index] - burst_buffer[burst_front_index];
  
  *mean_energy = 0.0;
  for(int burst_i = 0; burst_i < BURST_BUFFER_LEN; burst_i++)
    *mean_energy += burst_buffer[burst_i];
  *mean_energy /= BURST_BUFFER_LEN;
